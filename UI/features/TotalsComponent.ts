const TotalsComponent:ComponentFunction = (props?: Props) => {
    ...
    const tax_rate = 0.06
    const totals = () => { 
        const response = props!.cartArticles.reduce((sums, a:Article) => {
            const price_type: PriceType = props!.customer as CustomerType === "Regular Customer" ? "regular_price" : "member_price"  

            sums.subtotal += a[price_type]
            if (a.tax_status === "Taxable") sums.tax += a[price_type] * tax_rate
    ...
    const calculateChange:ButtonFunction = (e) => {
        e.preventDefault
        const cash = totalsReceivedCash.getValues()["cash"]
        const total = handleTotals().total
        if (total > cash) return window.alert("The provided cash is not enough")
    ...
    const generateReceipt:ButtonFunction = (e) => {
        e.preventDefault()
        const cart:Cart = {
